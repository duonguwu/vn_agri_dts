...
def create_record(metadata, time, loc_name, geo_level, item, metric, comp=None):
    ...
    alias_map = {
        ...
        "Bắc Trung bộ": "Bắc Trung Bộ", "Bắc Trung Bộ": "Bắc Trung Bộ", "Vùng Bắc Trung bộ": "Bắc Trung Bộ",
        ...
        "Hà Nội (mở rộng)": "Hà Nội", "Trung uơng": "Trung ương", "Tà Vi h": "Trà Vinh", "rn": "Trà Vinh"
    }
...
def parse_pl1():
    fpath = "/media/duongn/New Volume/UIT/aThacSy/Data Mining/2. Data Pre-processing/vn_agri_dts/segments/2012/2012_10_Phuluc_10_2012_PL1.md"
    metadata = {"year": 2012, "month": 10, "appendix_number": "PL1"}
    records = []
    t = {"year": 2012, "month": 10, "period_type": "Monthly", "report_date": "2012-10-15"}
    ...
    for row in rows:
        ...
        elif "Quốc gia" in name: loc = "Cả nước"
...
def parse_pl2():
    fpath = "/media/duongn/New Volume/UIT/aThacSy/Data Mining/2. Data Pre-processing/vn_agri_dts/segments/2012/2012_10_Phuluc_10_2012_PL2.md"
    metadata = {"year": 2012, "month": 10, "appendix_number": "PL2"}
    records = []
    t = {"year": 2012, "month": 10, "period_type": "Monthly", "report_date": "2012-10-15"}
    rows = extract_rows(fpath)
    ...
    for row in rows:
        ...
        name = row[0].replace("**", "").replace("ằ", "").replace("Thừa Thiên Huế", "Thừa Thiên Huế").strip()
        ...
        add(3, "Cây vụ đông", "Tổng số", "Area_Planted")
        ...
        add(8, "Khoai tây", None, "Area_Planted")
        add(9, "Cây khác", None, "Area_Planted")
        add(10, "Rau đậu", None, "Area_Planted")
    return records

def parse_pl3():
    fpath = "/media/duongn/New Volume/UIT/aThacSy/Data Mining/2. Data Pre-processing/vn_agri_dts/segments/2012/2012_10_Phuluc_10_2012_PL3.md"
    metadata = {"year": 2012, "month": 10, "appendix_number": "PL3"}
    records = []
    t = {"year": 2012, "month": 10, "period_type": "Monthly", "report_date": "2012-10-15"}
    rows = extract_rows(fpath)
    ...
    for row in rows:
        if "Tỉnh/TP" in row[0] or "Col" in row[0]: continue
        name = row[0].replace("**", "").replace("ằ", "").replace("T à Vi h", "Trà Vinh").replace("rn", "Trà Vinh").strip()
        if name == "" or "Diện tích" in name: continue
        if name == "rn": continue # artifact
        ...
        def add(idx, comm, sub, attr, unit="ha"):
            if idx >= len(row): return
            # Handle multi-line value like in PL3 row 48 (Trà Vinh)
            val_str = row[idx]
        ...
        add(4, "Lúa", "Mùa", "Area_Harvested")
        add(5, "Lúa", "Đông Xuân", "Area_Harvested") # In PL3, Col5 is "Đông xuân sớm 2013 DT thu hoạch"
...
def parse_pl4():
    fpath = "/media/duongn/New Volume/UIT/aThacSy/Data Mining/2. Data Pre-processing/vn_agri_dts/segments/2012/2012_10_Phuluc_10_2012_PL4.md"
    metadata = {"year": 2012, "month": 10, "appendix_number": "PL4"}
    records = []
    t = {"year": 2012, "month": 10, "period_type": "Monthly", "report_date": "2012-10-15"}
    rows = extract_rows(fpath)
...
if __name__ == "__main__":
    ...
    save_json({"metadata": {"year": 2012, "month": 10}, "records": parse_pl1()}, os.path.join(out_dir, "2012_10_Phuluc_10_2012_PL1.json"))
    save_json({"metadata": {"year": 2012, "month": 10}, "records": parse_pl2()}, os.path.join(out_dir, "2012_10_Phuluc_10_2012_PL2.json"))
    save_json({"metadata": {"year": 2012, "month": 10}, "records": parse_pl3()}, os.path.join(out_dir, "2012_10_Phuluc_10_2012_PL3.json"))
    save_json({"metadata": {"year": 2012, "month": 10}, "records": parse_pl4()}, os.path.join(out_dir, "2012_10_Phuluc_10_2012_PL4.json"))
    
    print("Successfully parsed Batch 1 for October 2012.")
