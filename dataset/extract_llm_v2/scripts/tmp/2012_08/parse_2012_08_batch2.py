...
def parse_pl6():
    fpath = "/media/duongn/New Volume/UIT/aThacSy/Data Mining/2. Data Pre-processing/vn_agri_dts/segments/2012/2012_08_Phuluc_08_2012_PL6.md"
    metadata = {"year": 2012, "month": 8, "appendix_number": "PL6"}
    records = []
    ...
    for row in rows:
        ...
        if val:
            records.append(create_record(metadata, t_8m, "Cả nước", "National", {"sector": "Forestry", "commodity": name}, {"attribute": "Value", "value": val, "unit": row[2].strip(), "data_type": "Estimate"}))
    return records

def parse_pl7():
    fpath = "/media/duongn/New Volume/UIT/aThacSy/Data Mining/2. Data Pre-processing/vn_agri_dts/segments/2012/2012_08_Phuluc_08_2012_PL7.md"
    metadata = {"year": 2012, "month": 8, "appendix_number": "PL7"}
    records = []
    t_8m = {"year": 2012, "month": 8, "period_type": "Cumulative", "report_date": "2012-08-31"}
    rows = extract_rows(fpath)
    for row in rows:
        ...
        def add(idx, comm, attr, unit="ha"):
            ...
            if val:
                records.append(create_record(metadata, t_8m, name, gl, {"sector": "Forestry", "commodity": comm}, {"attribute": attr, "value": val, "unit": unit, "data_type": "Actual"}))
...
def parse_pl8():
    fpath = "/media/duongn/New Volume/UIT/aThacSy/Data Mining/2. Data Pre-processing/vn_agri_dts/segments/2012/2012_08_Phuluc_08_2012_PL8.md"
    metadata = {"year": 2012, "month": 8, "appendix_number": "PL8"}
    records = []
    t_m = {"year": 2012, "month": 8, "period_type": "Monthly", "report_date": "2012-08-31"}
    t_8m = {"year": 2012, "month": 8, "period_type": "Cumulative", "report_date": "2012-08-31"}
    rows = extract_rows(fpath)
    for row in rows:
        if len(row) < 7: continue
        ...
        if v8:
            records.append(create_record(metadata, t_8m, "Cả nước", "National", {"sector": "Fishery", "commodity": name}, {"attribute": "Production", "value": v8, "unit": "1000_ton", "data_type": "Estimate"}))
    return records

def parse_pl9():
    fpath = "/media/duongn/New Volume/UIT/aThacSy/Data Mining/2. Data Pre-processing/vn_agri_dts/segments/2012/2012_08_Phuluc_08_2012_PL9.md"
    metadata = {"year": 2012, "month": 8, "appendix_number": "PL9"}
    records = []
    ...
    for row in rows:
        ...
        if "Danh mục" in name or "TỔNG CỘNG" in name or name == "" or name == "A" or name == "I" or name == "B" or "VỐN NGÂN SÁCH" in name:
             continue
        v7 = normalize_number(row[5])
        v8 = normalize_number(row[8])
        if v7:
            records.append(create_record(metadata, t_7m, "Cả nước", "National", {"sector": "Investment", "commodity": name}, {"attribute": "Investment_Amount", "value": v7, "unit": "million_VND", "data_type": "Actual"}))
        if v8:
            records.append(create_record(metadata, t_8m, "Cả nước", "National", {"sector": "Investment", "commodity": name}, {"attribute": "Investment_Amount", "value": v8, "unit": "million_VND", "data_type": "Estimate"}))
    return records

def parse_pl10a():
    fpath = "/media/duongn/New Volume/UIT/aThacSy/Data Mining/2. Data Pre-processing/vn_agri_dts/segments/2012/2012_08_Phuluc_08_2012_PL10a.md"
    metadata = {"year": 2012, "month": 8, "appendix_number": "PL10a"}
    records = []
    t_m = {"year": 2012, "month": 8, "period_type": "Monthly", "report_date": "2012-08-31"}
    t_8m = {"year": 2012, "month": 8, "period_type": "Cumulative", "report_date": "2012-08-31"}
    rows = extract_rows(fpath)
    ...
    for row in rows:
        ...
        q8 = normalize_number(row[9]); v8 = normalize_number(row[10])
        ...
        if q8: records.append(create_record(metadata, t_8m, "Cả nước", "National", {"sector": "Trade", "commodity": name}, {"attribute": "Volume", "value": q8, "unit": "1000_ton", "data_type": "Estimate", "trade_type": curr_trade}))
        if v8: records.append(create_record(metadata, t_8m, "Cả nước", "National", {"sector": "Trade", "commodity": name}, {"attribute": "Value", "value": v8, "unit": "million_USD", "data_type": "Estimate", "trade_type": curr_trade}))
    return records
...
if __name__ == "__main__":
    ...
    save_json({"metadata": {"year": 2012, "month": 8}, "records": parse_pl6()}, os.path.join(out_dir, "2012_08_Phuluc_08_2012_PL6.json"))
    save_json({"metadata": {"year": 2012, "month": 8}, "records": parse_pl7()}, os.path.join(out_dir, "2012_08_Phuluc_08_2012_PL7.json"))
    save_json({"metadata": {"year": 2012, "month": 8}, "records": parse_pl8()}, os.path.join(out_dir, "2012_08_Phuluc_08_2012_PL8.json"))
    save_json({"metadata": {"year": 2012, "month": 8}, "records": parse_pl9()}, os.path.join(out_dir, "2012_08_Phuluc_08_2012_PL9.json"))
    save_json({"metadata": {"year": 2012, "month": 8}, "records": parse_pl10a()}, os.path.join(out_dir, "2012_08_Phuluc_08_2012_PL10a.json"))
    
    print("Successfully parsed Batch 2 for August 2012.")
