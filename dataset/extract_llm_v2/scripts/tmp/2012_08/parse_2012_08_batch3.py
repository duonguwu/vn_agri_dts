...
def create_record(metadata, time, loc_name, geo_level, item, metric, comp=None):
    ...
    alias_map = {
        ...
        "THỤY SỸ": "Switzerland", "NAUY": "Norway", "AI XƠ LEN": "Iceland", "CHI LÊ": "Chile"
    }
...
def parse_compliance_lines(lines, metadata):
    records = []
    t = {"year": 2012, "month": 8, "period_type": "Monthly", "report_date": "2012-08-24"}
    for parts in lines:
...
def parse_pl12a_from_pl10c():
    fpath = "/media/duongn/New Volume/UIT/aThacSy/Data Mining/2. Data Pre-processing/vn_agri_dts/segments/2012/2012_08_Phuluc_08_2012_PL10c.md"
    if not os.path.exists(fpath): return []
    with open(fpath, 'r', encoding='utf-8') as f:
        content = f.read()
    
    # Use regex to find the mislabeled section header: # **_Phụ lục 11a_** or similar
    parts = re.split(r"#\s*\*+_*Phụ lục 11a_*", content)
    
    if len(parts) < 2: return []
    
    comp_section = parts[1]
    rows = []
    for line in comp_section.split("\n"):
        if "|" in line:
            p = [i.strip() for i in line.split("|")]
            if len(p) > 2 and p[0] == "" and p[-1] == "": rows.append(p[1:-1])
            elif len(p) > 1: rows.append(p)
            
    return parse_compliance_lines(rows, {"year": 2012, "month": 8, "appendix_number": "PL12a"})

def parse_pl12b():
    fpath = "/media/duongn/New Volume/UIT/aThacSy/Data Mining/2. Data Pre-processing/vn_agri_dts/segments/2012/2012_08_Phuluc_08_2012_PL12b.md"
    rows = extract_rows(fpath)
    return parse_compliance_lines(rows, {"year": 2012, "month": 8, "appendix_number": "PL12b"})
...
if __name__ == "__main__":
    ...
    t_7m_context = {"year": 2012, "month": 7, "period_type": "Cumulative", "report_date": "2012-07-31"}
    
    save_json({"metadata": {"year": 2012, "month": 8}, "records": parse_market_file("/media/duongn/New Volume/UIT/aThacSy/Data Mining/2. Data Pre-processing/vn_agri_dts/segments/2012/2012_08_Phuluc_08_2012_PL10b.md", {"year": 2012, "month": 8, "appendix_number": "PL10b"}, t_7m_context, "Export")}, os.path.join(out_dir, "2012_08_Phuluc_08_2012_PL10b.json"))
    save_json({"metadata": {"year": 2012, "month": 8}, "records": parse_market_file("/media/duongn/New Volume/UIT/aThacSy/Data Mining/2. Data Pre-processing/vn_agri_dts/segments/2012/2012_08_Phuluc_08_2012_PL10c.md", {"year": 2012, "month": 8, "appendix_number": "PL10c"}, t_7m_context, "Import")}, os.path.join(out_dir, "2012_08_Phuluc_08_2012_PL10c.json"))
    
    save_json({"metadata": {"year": 2012, "month": 8}, "records": parse_pl12a_from_pl10c()}, os.path.join(out_dir, "2012_08_Phuluc_08_2012_PL12a.json"))
    save_json({"metadata": {"year": 2012, "month": 8}, "records": parse_pl12b()}, os.path.join(out_dir, "2012_08_Phuluc_08_2012_PL12b.json"))
    
    print("Successfully parsed Batch 3 for August 2012.")
