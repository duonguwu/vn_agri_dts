...
def create_record(metadata, time, loc_name, geo_level, item, metric, comp=None):
    ...
    alias_map = {
        "Đồng bằng sông Cửu Long": "Đồng bằng sông Cửu Long", "ĐBS Cửu Long": "Đồng bằng sông Cửu Long", "ĐB sông Cửu Long": "Đồng bằng sông Cửu Long", "vùng ĐBSCL": "Đồng bằng sông Cửu Long",
        ...
        "Hà Nội (mở rộng)": "Hà Nội", "Cao Bng": "Cao Bằng", "ao ng": "Cao Bằng"
    }
...
def parse_pl1():
    fpath = "/media/duongn/New Volume/UIT/aThacSy/Data Mining/2. Data Pre-processing/vn_agri_dts/segments/2012/2012_09_Phuluc_09_2012_PL1.md"
    metadata = {"year": 2012, "month": 9, "appendix_number": "PL1"}
    records = []
    t = {"year": 2012, "month": 9, "period_type": "Monthly", "report_date": "2012-09-15"}
    ...
    for row in rows:
        ...
        elif "lúa thu đông" in lower_name: item.update({"commodity": "Lúa", "sub_item": "Thu Đông"})
        elif "cây lương thực" in lower_name: item.update({"commodity": "Cây lương thực có củ"})
        ...
        elif "mía" in lower_name: item.update({"commodity": "Mía"})
...
def parse_pl2():
    fpath = "/media/duongn/New Volume/UIT/aThacSy/Data Mining/2. Data Pre-processing/vn_agri_dts/segments/2012/2012_09_Phuluc_09_2012_PL2.md"
    metadata = {"year": 2012, "month": 9, "appendix_number": "PL2"}
    records = []
    t = {"year": 2012, "month": 9, "period_type": "Monthly", "report_date": "2012-09-15"}
    rows = extract_rows(fpath)
    ...
    for row in rows:
        ...
        add(3, "Lúa", "Hè Thu", "Area_Planted")
        add(4, "Lúa", "Hè Thu", "Area_Harvested")
...
def parse_pl3():
    fpath = "/media/duongn/New Volume/UIT/aThacSy/Data Mining/2. Data Pre-processing/vn_agri_dts/segments/2012/2012_09_Phuluc_09_2012_PL3.md"
    metadata = {"year": 2012, "month": 9, "appendix_number": "PL3"}
    records = []
    t = {"year": 2012, "month": 9, "period_type": "Monthly", "report_date": "2012-09-15"}
    rows = extract_rows(fpath)
    ...
    for row in rows:
        if "Tỉnh/TP" in row[0] or "Col" in row[0]: continue
        name = row[0].replace("**", "").replace("ằ", "").replace("C", "").replace("Bằ", "").strip() # Handle Cao Bằng artifacts
        if name == "" or name == "ao ng": name = "Cao Bằng"
        if name == "ao ng ,": name = "Cao Bằng"
        ...
        add(4, "Thuốc lá", None)
        add(5, "Mía", None)
        # Column 6 is "Cây khác" in general, but index 6 for PL3 is Cây khác for CCN
        add(6, "Cây khác", None)
        add(7, "Rau đậu các loại", None)
    return records

def parse_pl4():
    fpath = "/media/duongn/New Volume/UIT/aThacSy/Data Mining/2. Data Pre-processing/vn_agri_dts/segments/2012/2012_09_Phuluc_09_2012_PL4.md"
    metadata = {"year": 2012, "month": 9, "appendix_number": "PL4"}
    records = []
    t = {"year": 2012, "month": 9, "period_type": "Monthly", "report_date": "2012-09-15"}
    rows = extract_rows(fpath)
    ...
    for row in rows:
        ...
        add(1, "Lúa", "Thu Đông", "Area_Planted")
        add(2, "Lúa", "Mùa", "Area_Planted")
...
def parse_pl5():
    fpath = "/media/duongn/New Volume/UIT/aThacSy/Data Mining/2. Data Pre-processing/vn_agri_dts/segments/2012/2012_09_Phuluc_09_2012_PL5.md"
    metadata = {"year": 2012, "month": 9, "appendix_number": "PL5"}
    records = []
    t = {"year": 2012, "month": 9, "period_type": "Monthly", "report_date": "2012-09-15"}
    rows = extract_rows(fpath)
...
if __name__ == "__main__":
    ...
    save_json({"metadata": {"year": 2012, "month": 9}, "records": parse_pl1()}, os.path.join(out_dir, "2012_09_Phuluc_09_2012_PL1.json"))
    save_json({"metadata": {"year": 2012, "month": 9}, "records": parse_pl2()}, os.path.join(out_dir, "2012_09_Phuluc_09_2012_PL2.json"))
    save_json({"metadata": {"year": 2012, "month": 9}, "records": parse_pl3()}, os.path.join(out_dir, "2012_09_Phuluc_09_2012_PL3.json"))
    save_json({"metadata": {"year": 2012, "month": 9}, "records": parse_pl4()}, os.path.join(out_dir, "2012_09_Phuluc_09_2012_PL4.json"))
    save_json({"metadata": {"year": 2012, "month": 9}, "records": parse_pl5()}, os.path.join(out_dir, "2012_09_Phuluc_09_2012_PL5.json"))
    
    print("Successfully parsed Batch 1 for September 2012.")
