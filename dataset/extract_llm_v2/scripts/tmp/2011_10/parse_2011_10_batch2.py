...
def parse_pl5():
    metadata = {"year": 2011, "month": 10, "appendix_number": "PL5", "source_file": "2011_10_Phuluc_10_2011_f_PL5.md"}
    records = []
    t = {"year": 2011, "month": 10, "period_type": "Cumulative", "report_date": "2011-10-31"}
    data = [
        ("Diện tích rừng trồng mới tập trung", 149.7, "1000_ha"),
        ("Rừng phòng hộ, đặc dụng", 18.6, "1000_ha"),
        ("Rừng sản xuất", 131.1, "1000_ha"),
        ("Diện tích rừng trồng được chăm sóc", 319.2, "1000_ha"),
        ("Số cây lâm nghiệp trồng phân tán", 170.5, "million_trees"),
        ("Diện tích rừng được khoanh nuôi tái sinh", 698.0, "1000_ha"),
        ("Diện tích rừng được khoán bảo vệ", 2333.7, "1000_ha"),
        ("Sản lượng gỗ", 3530.0, "1000_m3")
    ]
...
def parse_pl6():
    fpath = "/media/duongn/New Volume/UIT/aThacSy/Data Mining/2. Data Pre-processing/vn_agri_dts/segments/2011/2011_10_Phuluc_10_2011_f_PL6.md"
    rows = extract_rows_from_file(fpath)
    metadata = {"year": 2011, "month": 10, "appendix_number": "PL6", "source_file": "2011_10_Phuluc_10_2011_f_PL6.md"}
    records = []
    t = {"year": 2011, "month": 10, "period_type": "Cumulative", "report_date": "2011-10-31"}
    ...
    for row in rows:
        ...
        if name in ["Cả nước", "Miền bắc", "ĐB. sông Hồng", "Trung du và MN phía Bắc", "Bắc Trung Bộ", 
                    "Miền Nam", "D.H Nam Trung Bộ", "Tây Nguyên", "Đông Nam Bộ", "ĐB. sông Cửu Long", "Trung ương"]:
...
def parse_pl7():
    metadata = {"year": 2011, "month": 10, "appendix_number": "PL7", "source_file": "2011_10_Phuluc_10_2011_f_PL7.md"}
    records = []
    t_9m = {"year": 2011, "month": 9, "period_type": "Cumulative", "report_date": "2011-09-30"}
    t_month = {"year": 2011, "month": 10, "period_type": "Monthly", "report_date": "2011-10-31"}
    t_10m = {"year": 2011, "month": 10, "period_type": "Cumulative", "report_date": "2011-10-31"}
    
    # [Item, v9c, vm, v10c]
    data = [
        ("Tổng sản lượng", 4136.0, 476.0, 4612.0),
        ("Sản lượng khai thác", 1973.0, 226.0, 2199.0),
        ("Khai thác biển", 1833.0, 206.0, 2039.0),
        ("Khai thác nội địa", 140.0, 20.0, 160.0),
        ("Sản lượng nuôi trồng", 2163.0, 250.0, 2413.0)
    ]
    for item, v9c, vm, v10c in data:
        records.append(create_record(metadata, t_9m, "Cả nước", "National", {"sector": "Fishery", "commodity": item}, {"attribute": "Production", "value": v9c, "unit": "1000_ton", "data_type": "Actual"}))
        ...
        records.append(create_record(metadata, t_10m, "Cả nước", "National", {"sector": "Fishery", "commodity": item}, {"attribute": "Production", "value": v10c, "unit": "1000_ton", "data_type": "Estimate"}))
    return records

def parse_pl8():
    fpath = "/media/duongn/New Volume/UIT/aThacSy/Data Mining/2. Data Pre-processing/vn_agri_dts/segments/2011/2011_10_Phuluc_10_2011_f_PL8.md"
    rows = extract_rows_from_file(fpath)
    metadata = {"year": 2011, "month": 10, "appendix_number": "PL8", "source_file": "2011_10_Phuluc_10_2011_f_PL8.md"}
    records = []
    t = {"year": 2011, "month": 10, "period_type": "Cumulative", "report_date": "2011-10-20"}
    
    for row in rows:
        if len(row) < 8: continue
        name = row[0].replace("**", "").replace("_", "").replace("*", "").strip()
...
if __name__ == "__main__":
    ...
    save_json({"metadata": {"year": 2011, "month": 10}, "records": parse_pl5()}, os.path.join(out_dir, "2011_10_Phuluc_10_2011_f_PL5.json"))
    save_json({"metadata": {"year": 2011, "month": 10}, "records": parse_pl6()}, os.path.join(out_dir, "2011_10_Phuluc_10_2011_f_PL6.json"))
    save_json({"metadata": {"year": 2011, "month": 10}, "records": parse_pl7()}, os.path.join(out_dir, "2011_10_Phuluc_10_2011_f_PL7.json"))
    save_json({"metadata": {"year": 2011, "month": 10}, "records": parse_pl8()}, os.path.join(out_dir, "2011_10_Phuluc_10_2011_f_PL8.json"))
    
    print("Successfully parsed Batch 2 (PL5-PL8) for October 2011.")
