...
def parse_pl5():
    metadata = {"year": 2011, "month": 11, "appendix_number": "PL5", "source_file": "2011_11_Phuluc_11_2011_f_PL5.md"}
    records = []
    t = {"year": 2011, "month": 11, "period_type": "Cumulative", "report_date": "2011-11-30"}
    data = [
        ("Diện tích rừng trồng mới tập trung", 201.8, "1000_ha"),
        ("Rừng phòng hộ, đặc dụng", 18.6, "1000_ha"),
        ("Rừng sản xuất", 183.2, "1000_ha"),
        ("Diện tích rừng trồng được chăm sóc", 322.7, "1000_ha"),
        ("Số cây lâm nghiệp trồng phân tán", 176.2, "million_trees"),
        ("Diện tích rừng được khoanh nuôi tái sinh", 698.5, "1000_ha"),
        ("Diện tích rừng được khoán bảo vệ", 2423.4, "1000_ha"),
        ("Sản lượng gỗ", 3968.0, "1000_m3")
    ]
...
def parse_pl6():
    fpath = "/media/duongn/New Volume/UIT/aThacSy/Data Mining/2. Data Pre-processing/vn_agri_dts/segments/2011/2011_11_Phuluc_11_2011_f_PL6.md"
    rows = extract_rows(fpath)
    metadata = {"year": 2011, "month": 11, "appendix_number": "PL6", "source_file": "2011_11_Phuluc_11_2011_f_PL6.md"}
    records = []
    t = {"year": 2011, "month": 11, "period_type": "Cumulative", "report_date": "2011-11-30"}
...
def parse_pl7():
    metadata = {"year": 2011, "month": 11, "appendix_number": "PL7", "source_file": "2011_11_Phuluc_11_2011_f_PL7.md"}
    records = []
    ...
    # [Item, v10c, vm, v11c]
    data = [
        ("Tổng sản lượng", 4573.0, 436.0, 5009.0),
        ("Sản lượng khai thác", 2160.0, 190.0, 2350.0),
        ("Khai thác biển", 2000.0, 173.0, 2173.0),
        ("Khai thác nội địa", 160.0, 17.0, 177.0),
        ("Sản lượng nuôi trồng", 2413.0, 246.0, 2659.0)
    ]
    for item, v10c, vm, v11c in data:
        records.append(create_record(metadata, t_10m, "Cả nước", "National", {"sector": "Fishery", "commodity": item}, {"attribute": "Production", "value": v10c, "unit": "1000_ton", "data_type": "Actual"}))
        ...
        records.append(create_record(metadata, t_11m, "Cả nước", "National", {"sector": "Fishery", "commodity": item}, {"attribute": "Production", "value": v11c, "unit": "1000_ton", "data_type": "Estimate"}))
    return records

def parse_pl8():
    fpath = "/media/duongn/New Volume/UIT/aThacSy/Data Mining/2. Data Pre-processing/vn_agri_dts/segments/2011/2011_11_Phuluc_11_2011_f_PL8.md"
    rows = extract_rows(fpath)
    metadata = {"year": 2011, "month": 11, "appendix_number": "PL8", "source_file": "2011_11_Phuluc_11_2011_f_PL8.md"}
    records = []
    t = {"year": 2011, "month": 11, "period_type": "Cumulative", "report_date": "2011-11-20"}
...
if __name__ == "__main__":
    ...
    save_json({"metadata": {"year": 2011, "month": 11}, "records": parse_pl5()}, os.path.join(out_dir, "2011_11_Phuluc_11_2011_f_PL5.json"))
    save_json({"metadata": {"year": 2011, "month": 11}, "records": parse_pl6()}, os.path.join(out_dir, "2011_11_Phuluc_11_2011_f_PL6.json"))
    save_json({"metadata": {"year": 2011, "month": 11}, "records": parse_pl7()}, os.path.join(out_dir, "2011_11_Phuluc_11_2011_f_PL7.json"))
    save_json({"metadata": {"year": 2011, "month": 11}, "records": parse_pl8()}, os.path.join(out_dir, "2011_11_Phuluc_11_2011_f_PL8.json"))
    
    print("Successfully parsed Batch 2 (PL5-PL8) for November 2011.")
